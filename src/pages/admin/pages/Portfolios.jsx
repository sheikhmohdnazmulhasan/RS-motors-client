...
const Portfolios = () => {
  ...
  document.title = `RS Motors | All Portfolio`

    const { data = [], refetch } = useQuery({
        queryKey: ['portfolios'],
        queryFn: async () => {
            const response = await axios.get('https://rs-motors-server.vercel.app/portfolios/v1');
    ...
    function handleDelete(_id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`https://rs-motors-server.vercel.app/portfolio-delete/v1/${_id}`).then(data => {
